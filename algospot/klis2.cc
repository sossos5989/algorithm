...
struct LDS {
    ...
    void push(int i) { prev[prevCnt++] = i; }
};
...
bool indexComp(int left, int right) { return seq[left] < seq[right]; }
...
int main() {
    ...
    while (c-- > 0) {
        ...
        for (int i = n - 1; i >= 0; --i) {
            int lmax = 0;
            for (int j = n - 1; j > i; --j) {
                if (seq[i] < seq[j]) lmax = max(lmax, lds[j].len);
            }
            lds[i].len = lmax + 1;
            ...
            for (int j = n - 1; j > i; --j) {
                if (seq[i] < seq[j] && lds[j].len == lmax) {
                    lds[i].cnt += lds[j].cnt;
                    if (lds[i].cnt < 0) lds[i].cnt = INT_MAX;
                    lds[i].push(j);
                }
            }

            if (lds[i].cnt == 0) lds[i].cnt = 1;
        }
        ...
        sortedCnt = 0;
        for (int i = 0; i < n; ++i) {
            if (lds[i].len == maxLen) sorted[sortedCnt++] = i;
        }
